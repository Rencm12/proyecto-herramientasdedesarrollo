...
import { accesorios as listaDeAccesorios } from "../../data/accesorios"; 
import Card from "./Card-accesorios"; 
...
function Accesorios() {
  ...
  const [etiqueta, setEtiqueta] = useState("Todas"); // NUEVO ESTADO
  ...
  const limpiarFiltros = () => {
    ...
    setPlataforma("Todas");
    setCategoria("Todas");
    setEtiqueta("Todas"); // Limpiar también el nuevo filtro
  };
  ...
  const productosProcesados = useMemo(() => {
    let filtrados = listaDeAccesorios.filter((item) => {
      ...
      // Lógica para el nuevo filtro de etiquetas
    ...
    return filtrados;
  }, [busqueda, plataforma, categoria, etiqueta, orden]); // Añadido 'etiqueta' a las dependencias

  return (
    <div className="min-h-screen bg-[#0f1115] p-6 md:p-10 font-sans">
      
      <div className="mb-10 max-w-[1400px] mx-auto">
        <CarruselAccesorios />
      </div>

      <h1 className="text-4xl text-[#00ffc3] font-bold text-center mb-6">
